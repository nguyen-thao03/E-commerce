  ...
  try {
    ...
    if (sellerEmail) {
      ...
      fs.unlink(filePath, (err) => {
        if (err) {
          console.log(err);
          res.status(500).json({ message: "Lỗi xóa tập tin" });
        }
      });
    ...
    try {
      await sendMail({
        email: seller.email,
        subject: "Kích hoạt cửa hàng của bạn",
        message: `Xin chào ${seller.name}, vui lòng nhấp vào liên kết để kích hoạt cửa hàng của bạn: ${activationUrl}`,
      });
      res.status(201).json({
        success: true,
        message: `vui lòng kiểm tra email của bạn:- ${seller.email} để kích hoạt cửa hàng của bạn!`,
      });
    } catch (error) {
...
router.delete(
  ...
  catchAsyncErrors(async (req, res, next) => {
    try {
      ...
      res.status(201).json({
        success: true,
        message: "Xóa người bán thành công!",
      });
    } catch (error) {
      return next(new ErrorHandler(error.message, 500));
    }
